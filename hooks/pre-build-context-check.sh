...
if ! command -v jq >/dev/null 2>&1; then
  message="Pre-build context hook is unavailable because jq is not installed. Install jq before relying on this guardrail."
  if [[ "${PLANNING_HOOK_STRICT:-0}" == "1" ]]; then
    printf '%s\n' "$message" >&2
    exit 2
  fi
  printf '%s\n' '{"hookSpecificOutput":{"hookEventName":"PreToolUse","additionalContext":"Pre-build context hook is unavailable because jq is not installed. Install jq before relying on this guardrail."}}'
  exit 0
fi
