...
4. Assign state IDs such as `ST-01`, while preserving the source breadboard IDs.
...
8. Add initial, terminal, retry, cancellation, timeout, and failure states only when specified or clearly mark them as missing.
...
| --- | --- | --- | --- | --- | --- |
| ST-01 | P2 | Autopay setup | Billing | User is configuring Autopay | specified |

Use `Status` values:

- `specified` — directly supported by the breadboard
...
| --- | --- | --- | --- | --- | --- | --- | --- | --- |
| TR-01 | ST-01 | user | Submit | Details valid | Request authorization | ST-02 | U4 → N4 | specified |
...
stateDiagram-v2
    ST_01: Autopay setup
    ST_02: Authorization pending
    ST_03: Autopay active
    ST_04: Authorization failed

    ST_01 --> ST_02: Submit [details valid]
    ST_02 --> ST_03: Provider approves
    ST_02 --> ST_04: Timeout
```

Mermaid identifiers may replace hyphens with underscores, but labels and tables must retain the canonical IDs.
