...
from validation_schema import (
    ANALYSIS_VALIDATION, BRANCH_INDEPENDENCE, BRIEF_FIELD_STATUS, CONCEPT_SELECTION_STATUS,
    CONTRAST_CASE_TYPES, COVERAGE, DETERMINISTIC_VALIDATION, DISCOVERY_PATH, EPISTEMIC,
    EVIDENCE_BASIS, EVIDENCE_COMPLETION, EVIDENTIARY_ROLE, EXECUTION_LEVEL, FINGERPRINT_PATTERN,
    FIT_STATUS, FIXTURE_TYPE, GATE, HUMAN_REVIEW, HYPOTHESIS_STATUS, ID_PATTERNS, INDEPENDENCE,
    INTERPRETATION_COMPLETION, INTERPRETIVE_STATUS, LINEAGE_RELATIONSHIP, LIST_REGISTRIES,
    LU_STATUS, MODEL_CHECK, OBSERVABILITY_RESOLUTION, OBSERVABILITY_STATUS, PHASES, PROPAGATION,
    REJECTION_LAYER, REQUIREMENT_ORIGIN, ROTATION_STATUS, SEARCH_TYPE, SHAPING_FRAME_STATUS,
    SOURCE_CONTENT_TRUST, SOURCE_INSTRUCTION_RISK, STUDY_STATUS, SUFFICIENCY, SUFFICIENCY_DIMENSIONS,
    SUFFICIENCY_REPAIR, TRACE_BASIS, TRACE_STATUS, TRANSFERABILITY,
    as_list, as_string_list, ids, load, refs_exist, require_bool, require_nonempty_string,
    require_string_list, valid_enum, validate_action, validate_registry_enums, validate_registry_fields,
)
from validation_state import validate_freeze_state, validate_manifest_state, validate_sufficiency_state
...
def main() -> int:
    ...
    registry_values = {
        ...
        "sufficiency.json": sufficiency,
        "freeze.json": freeze,
        "decision_outcome.json": decision_outcome,
    ...
    for registry_name, registry_value in registry_values.items():
        validate_registry_fields(registry_name, registry_value, errors)
        validate_registry_enums(registry_name, registry_value, errors)
    ...
    validate_sufficiency_state(sufficiency, all_structured_refs, pyramid_ids, errors)
    validate_freeze_state(
            root, freeze, sufficiency, episodes, evidence, independent_lineage_count,
            observability, analysis_run_by_id, errors,
        )
    if (
    ...
    decision_brief_text = validate_manifest_state(
            root, manifest, mode, fixture_type, is_synthetic_fixture, study_status, evidence,
            concepts, findings, needs, principles, errors,
        )
